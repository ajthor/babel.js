...
var backpropagation = module.exports = new system();

backpropagation.prototype.train = function(inputs, ideals) {
	var err = 1, index;
...
backpropagation.prototype._iteration = function(input, ideal) {
	try {
		...
		for(i = 0; i < this.network._layers.length; i++) {
			...
			for(j = 0; j < this.network._layers[i]._neurons.length; j++) {
				...
				for(k = 0; k < neuron.weights.length; k++) {
					...
					neuron.deltas[k] = this.network.options.learningRate * neuron.gradient * (this._layers[i-1] ? this._layers[i-1]._neurons[k].output : input[k]);
